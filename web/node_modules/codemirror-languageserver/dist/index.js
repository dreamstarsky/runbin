...
const blockPedantic = { ...blockNormal,
  html: edit('^ *(?:comment *(?:\\n|\\s*$)' + '|<(tag)[\\s\\S]+?</\\1> *(?:\\n{2,}|\\s*$)' // closed tag
...
const punctuation = edit(/^((?![*_])punctSpace)/, 'u').replace(/punctSpace/g, _punctuationOrSpace).getRegex(); // GFM allows ~ inside strong and em for strikethrough

const _punctuationGfmStrongEm = /(?!~)[\p{P}\p{S}]/u;
const _punctuationOrSpaceGfmStrongEm = /(?!~)[\s\p{P}\p{S}]/u;
const _notPunctuationOrSpaceGfmStrongEm = /(?:[^\s\p{P}\p{S}]|~)/u; // sequences em should skip over [title](link), `code`, <html>
...
const emStrongRDelimAstGfm = edit(emStrongRDelimAstCore, 'gu').replace(/notPunctSpace/g, _notPunctuationOrSpaceGfmStrongEm).replace(/punctSpace/g, _punctuationOrSpaceGfmStrongEm).replace(/punct/g, _punctuationGfmStrongEm).getRegex(); // (6) Not allowed for _
...
const inlinePedantic = { ...inlineNormal,
  ...
  blockquote(src) {
    ...
    if (cap) {
      ...
      while (lines.length > 0) {
        ...
        text = text ? `${text}\n${currentText}` : currentText; // parse blockquote lines as top level tokens
        // merge paragraphs if this is a continuation

        const top = this.lexer.state.top;
        this.lexer.state.top = true;
        this.lexer.blockTokens(currentText, tokens, true);
        this.lexer.state.top = top; // if there is no continuation then we are done
    ...
    if (cap) {
      ...
      while (src) {
        ...
        if (!endEarly) {
          ...
          const htmlBeginRegex = this.rules.other.htmlBeginRegex(indent); // Check if following lines should be included in List Item
  ...
  emStrong(src, maskedSrc, prevChar = '') {
    ...
    if (!nextChar || !prevChar || this.rules.inline.punctuation.exec(prevChar)) {
      ...
      while ((match = endReg.exec(maskedSrc)) != null) {
        ...
        rLength = Math.min(rLength, rLength + delimTotal + midDelimTotal); // char length can be >1 for unicode characters;

        const lastCharLength = [...match[0]][0].length;
        const raw = src.slice(0, lLength + match.index + lastCharLength + rLength); // Create `em` if smallest delimiter has odd char count. *a***
...
class _Lexer {
  ...
  blockTokens(src, tokens = [], lastParagraphClipped = false) {
    ...
    while (src) {
      ...
      if (token = this.tokenizer.code(src)) {
        src = src.substring(token.raw.length);
        const lastToken = tokens.at(-1); // An indented code block cannot interrupt a paragraph.
  ...
  inlineTokens(src, tokens = []) {
    ...
    let maskedSrc = src;
    let match = null; // Mask out reflinks
...
class _Parser {
  ...
  parse(tokens, top = true) {
    ...
    for (let i = 0; i < tokens.length; i++) {
      const anyToken = tokens[i]; // Run any renderer extensions
...
class Marked {
    ...
    args.forEach(pack => {
      ...
      if (pack.renderer) {
        ...
        for (const prop in pack.renderer) {
          ...
          const prevRenderer = renderer[rendererProp]; // Replace renderer with func to run extension, but fall back if false
      ...
      if (pack.tokenizer) {
        ...
        for (const prop in pack.tokenizer) {
          ...
          const prevTokenizer = tokenizer[tokenizerProp]; // Replace tokenizer with func to run extension, but fall back if false
  ...
  parseMarkdown(blockType) {
    ...
    const parse = (src, options) => {
      ...
      const throwError = this.onError(!!opt.silent, !!opt.async); // throw error if an extension set async to true but parse was called with async: false
...
function isLSPTextEdit(textEdit) {
    return (textEdit === null || textEdit === void 0 ? void 0 : textEdit.range) !== undefined;
}
